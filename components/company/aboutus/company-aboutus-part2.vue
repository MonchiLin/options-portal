  ...
  <the-part>
    ...
    >
      <div
        ...
        :key="index"
        class="<md:(w-full mt-[26px]) w-[30%] py-[35px] px-[24px] h-[266px]"
        :style="item.style"
        style="box-shadow: 0px 13px 19px rgba(0, 0, 0, 0.07)"
      >
        <img class="w-[44px] h-[44px]" :src="item.icon" alt="" />
        <p class="text-[24px] font-black mt-[10px]">{{ item.title }}</p>
        <p class="text-[14px] leading-[20px] mt-[10px]">
          {{ item.description }}
        </p>
      </div>
    </div>
    ...
    const cardData = reactive([
      {
        ...
        icon: "/company/aboutus/part2/Integrity.png",
        description:
          "We uphold the highest standards of regulation and compliance being regulated by the CySEC in the European Union and complying with ESMA regulations \n",
