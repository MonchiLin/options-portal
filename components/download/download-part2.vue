<template>
  <the-part
      background-for-md="/download/part2/bg_m.png"
      background="/download/part2/bg.png"
  >
    <div
        class="
          <md:(flex-col pb-[14px] pt-[20px] h-auto justify-start)
          justify-between pb-[38px] pt-[60px]
    ...
    >
      <div
        ...
        <h6 class="text-primary text-[35px] font-black <md:(text-[20px])">{{ text.t1 }}</h6>
        <h5 class="pt-[10px] text-[#18191F] text-[18px] leading-[32px] <md:(text-[14px] leading-[20px])">{{ text.t2 }}</h5>

        <ul class="mt-[30px] text-secondary font-medium">
          <li v-for="item of text.t3" :key="item" class="flex flex-row items-center mt-[16px] ">
            <blue-pointer/>
        ...
        <!--   按钮   -->
        <button
            class="<md:(mt-[30px]) text-[16px] leading-[24px] mt-[80px] w-[193px] bg-orange text-white rounded-[6px] py-[12px] flex items-center justify-center">
          {{ text.t4 }}
        </button>
